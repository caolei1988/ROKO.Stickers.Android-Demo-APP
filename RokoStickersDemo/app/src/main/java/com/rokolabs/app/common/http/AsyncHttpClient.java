...
package com.rokolabs.app.common.http;
...
public class AsyncHttpClient {
	public interface RequestIntercepter {
		public HttpResponse handResposne(HttpResponse resp, AbstractHttpClient client, HttpUriRequest request,
										 HttpContext context) throws ClientProtocolException, IOException;
	}
	
	public static RequestIntercepter sIntercepter;
	...
	private static final int DEFAULT_SOCKET_BUFFER_SIZE = 32 * 1024;

	private static int maxConnections = DEFAULT_MAX_CONNECTIONS;
	private static int maxTotalConnections = 10 * DEFAULT_MAX_CONNECTIONS;
	private static int socketTimeout = DEFAULT_SOCKET_TIMEOUT;
	private static int connTimeout = DEFAULT_CONNECTION_TIMEOUT;
	...
	public static String HEADER_API_KEY_CONTENT = "Od5AKuTjJzihQPIjx4IVB+VAAmH/03AswAd7pp/KQEE=";
	public static String HEADER_AUTH_SESSION = null;

	private static final String HEADER_API_KEY = "X-ROKO-Mobi-Api-Key";
	private static final String HEADER_AUTH_KEY = "X-ROKO-Mobi-User-Session";
	private static final String HEADER_AUTHORIZATION = "Authorization";
	private static final String HEADER_DEVICE_TYPE = "X-ROKO-Mobi-Device-Type";
	private static final String HEADER_DEVICE_MODEL = "X-ROKO-Mobi-Device-Model";
	private static final String HEADER_OS = "X-ROKO-Mobi-OS";
	private static final String HEADER_OS_VER = "X-ROKO-Mobi-OS-Version";
	private static final String HEADER_APP_VER = "X-ROKO-Mobi-Application-Version";
	private static final String HEADER_CARRIER = "X-ROKO-Mobi-Carrier";
	private static final String HEADER_SDK_VERSION = "X-ROKO-Mobi-SDK-Version";
	private static final String HEADER_TIMEZONE = "X-ROKO-Mobi-Timezone";
	private static final String HEADER_NETWORK_TYPE = "X-ROKO-Mobi-Network-Type";
	/**
	...
	public AsyncHttpClient() {
		...
		httpClient.addRequestInterceptor(new HttpRequestInterceptor() {
			public void process(HttpRequest request, HttpContext context) {
				...
				if (!request.containsHeader(HEADER_API_KEY)) {
					request.addHeader(HEADER_API_KEY, HEADER_API_KEY_CONTENT);
				}
				if (!TextUtils.isEmpty(HEADER_AUTH_SESSION)
						&& !request.containsHeader(HEADER_AUTHORIZATION) && Util.isRokoRequest(request)) {
					request.addHeader(HEADER_AUTHORIZATION, HEADER_AUTH_KEY + " "
							+ HEADER_AUTH_SESSION);
				}
	...
	public void getURI(Context context, String url, Header[] headers,
					   RequestParams params, AsyncHttpResponseHandler responseHandler) {
		HttpUriRequest request = null;
		try {
			request = new HttpGet(new URI(getUrlWithQueryString(url, params)));
	...
	public HttpEntityEnclosingRequestBase addEntityToRequestBase(HttpEntityEnclosingRequestBase requestBase, HttpEntity entity) {
		if (entity != null) {
	...
	public static String parseResponse(HttpResponse response) throws IOException {
		return parseResponse(response, "UTF-8", false);
	}
	
	public static String parseResponse(HttpResponse response, String defaultEncoding) throws IOException {
		return parseResponse(response, defaultEncoding, false);
	}
	
	public static String parseResponse(HttpResponse response, String defaultEncoding, boolean returnError) throws IOException {
		...
		try {
			...
			if (temp != null) {
				...
				responseBody = EntityUtils.toString(entity, defaultEncoding);
			}
		} catch (IOException e) {
			e.printStackTrace();
			return null;
		...
		if (status.getStatusCode() >= 300) {
			Log.d(AsyncHttpClient.class.getSimpleName(), status.toString());
			Log.d(AsyncHttpClient.class.getSimpleName(), ""+responseBody);
			if(returnError)
				return responseBody;
			return null;
		} else {
	...
	public String syncGet(String url, Header[] headers) throws ClientProtocolException, IOException {
		HttpGet get = new HttpGet(url);
		get.setHeaders(headers);
		return parseResponse(syncExec(get));
	}
	
	public String syncGet(String url, RequestParams params, Header[] headers) throws ClientProtocolException, IOException {
		HttpGet get = new HttpGet(getUrlWithQueryString(url, params));
		get.setHeaders(headers);
		return parseResponse(syncExec(get));
	}
	
	public String syncGet(String url, RequestParams params, Header[] headers, String defaultEncoding)
			throws ClientProtocolException, IOException {
		HttpGet get = new HttpGet(getUrlWithQueryString(url, params));
		get.setHeaders(headers);
		return parseResponse(syncExec(get), defaultEncoding);
	}
	
	public String syncGet(String url, RequestParams params) throws ClientProtocolException, IOException {
		return parseResponse(syncExec(new HttpGet(getUrlWithQueryString(url, params))));
	}
	
	public String syncGet(String url, RequestParams params, String defaultEncoding)
			throws ClientProtocolException, IOException {
		return parseResponse(syncExec(new HttpGet(getUrlWithQueryString(url, params))),
				defaultEncoding);
	}
	
	public HttpResponse syncGetRaw(String url, RequestParams params) throws ClientProtocolException, IOException {
		return syncExec(new HttpGet(getUrlWithQueryString(url, params)));
	}
	...
	public String syncPost(String url, Header[] headers, HttpEntity entity, String contentType)
			throws ClientProtocolException, IOException {
		return syncPost(url, headers, entity, contentType, "UTF-8");
	}
	...
	public String syncPost(String url, Header[] headers, HttpEntity entity)
			throws ClientProtocolException, IOException {
		HttpEntityEnclosingRequestBase request = addEntityToRequestBase(new HttpPost(url), entity);
		request.setHeaders(headers);
		return parseResponse(syncExec(request));
	}
	
	public String syncPost(String url, HttpEntity entity) throws ClientProtocolException, IOException {
		return parseResponse(syncExec(addEntityToRequestBase(new HttpPost(url),entity)));
	}
	
	public String syncPost(String url, RequestParams params) throws ClientProtocolException, IOException {
		return syncPost(url, paramsToEntity(params));
	}

	public HttpResponse syncExec(HttpUriRequest request) throws ClientProtocolException, IOException {
		HttpResponse resp = httpClient.execute(request, httpContext);
		if(sIntercepter != null)
			resp = sIntercepter.handResposne(resp, httpClient, request, httpContext);
		return resp;
	}
