...
import com.rokolabs.app.common.util.IOUtils;
...
public class ImageCache
{
    ...
    private static ExecutorService mSavingThreadPool = Executors.newSingleThreadExecutor();
    /**
    ...
    public void saveBitmapToDiskSync(String data, File bitmap)
    {
        synchronized (mDiskCacheLock)
        {
            // Add to disk cache
                ...
                try
                {
                    ...
                    if (snapshot == null)
                    {
                        ...
                        if (editor != null)
                        {
                            ...
                            InputStream in = new BufferedInputStream(new FileInputStream(bitmap));
                            IOUtils.streamCopy(in, out);
                            editor.commit();
                        }
                    } else
                    {
    ...
    public void saveBitmapToDiskSync(String data, Bitmap bitmap) {
        if (data == null || bitmap == null) {
            return;
        }
	...
	public boolean hasDiskCacheFor(String data) {
		final String key = hashKeyForDisk(data);
		synchronized (mDiskCacheLock) {
			...
			if (mDiskLruCache != null) {
				return mDiskLruCache.has(key, DISK_CACHE_INDEX);
			}
			return false;
		}
	}
    
	public File getCachedFile(String data) {
		final String key = hashKeyForDisk(data);
		synchronized (mDiskCacheLock) {
			while (mDiskCacheStarting) {
				try {
			...
			if (mDiskLruCache != null) {
				return mDiskLruCache.get(key, DISK_CACHE_INDEX);
			}
			return null;
		}
	}
	
	public void removeCachedFile(String data) {
		final String key = hashKeyForDisk(data);
		synchronized (mDiskCacheLock) {
			while (mDiskCacheStarting) {
				try {
    ...
    {
        return getBitmapFromDiskCache(data, 0, 0);
    }
        ...
        synchronized (mDiskCacheLock)
        {
            ...
            if (mDiskLruCache != null)
            {
                ...
                try
                {
                    ...
                    Logger.printMemory("OOM in load bitmap from disk");
    ...
    /**
     * For temporary reduce cache size in case need to do some operations that requires a lot memory 
     * @param percentRemain
     */
    public void trimMemoryCache(float percentRemain){
        if (mMemoryCache != null)
        {
            mMemoryCache.trimToSize((int) (mMemoryCache.maxSize()*percentRemain));
            if (Utils.DEBUG)
            {
                Log.d(TAG, "Memory cache size reduced to "+percentRemain);
            }
        }
    }
...
//            }
            ...
            Log.d(TAG, "memory cache size = "+memCacheSize/1024/1024+"MB");
        }
