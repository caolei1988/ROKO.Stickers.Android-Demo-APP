...
public abstract class AsyncTask<Params, Progress, Result>
{
	...
	public static int priorityThreshold = 3;
    ...
    private static final BlockingQueue<Runnable> sPoolWorkQueue        = new LinkedLifoBlockingDeque<Runnable>();
    ...
    private final WorkerFutureTask<Result>             mFuture;
    ...
    public AsyncTask()
    {
        ...
        mFuture = new WorkerFutureTask<Result>(mWorker)
        {
            @Override
            protected void done()
            {
        ...
        mFuture.setPriority(System.currentTimeMillis());
    }
    ...
    public final void renewTaskPriority(){
    	mFuture.setPriority(System.currentTimeMillis());
    }
    ...
    private static abstract class WorkerFutureTask<Result> extends FutureTask<Result> implements Comparable<WorkerFutureTask<Result>> {
    	public WorkerFutureTask(Callable<Result> callable) {
			super(callable);
		}
    	...
    	public void setPriority(long priority){
    		this.priority = (int) ((priority/1000) & 0x7FFFFFFF);
    	}
    	
		@Override
		public int compareTo(WorkerFutureTask<Result> another) 
		{
			int diff = another.priority - priority;
			if (Math.abs(diff) < priorityThreshold)
				return -diff;
