...
class AsyncHttpRequest implements Runnable {
	...
	private void makeRequest() throws IOException {
		if (!Thread.currentThread().isInterrupted()) {
			Logger.d(request.getMethod() + " " + request.getURI());
			long start = SystemClock.elapsedRealtime();
			HttpResponse response = client.execute(request, context);
			if(AsyncHttpClient.sIntercepter != null)
				response = AsyncHttpClient.sIntercepter.handResposne(response, client, request, context);
			Logger.d((SystemClock.elapsedRealtime() - start) + "ms, response: "
					+ response.getStatusLine() + ", for " + request.getMethod() + " "
					+ request.getURI());
			if (!Thread.currentThread().isInterrupted()) {
				if (responseHandler != null) {
	...
	private void makeRequestWithRetries() throws ConnectException {
		...
		while (retry) {
			try {
				...
				if (responseHandler != null) {
					responseHandler.sendFailureMessage(e, e.getLocalizedMessage());
				}
				return;
